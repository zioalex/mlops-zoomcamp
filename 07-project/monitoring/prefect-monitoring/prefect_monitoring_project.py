...
REFERENCE_DATA_FILE = "data/cleaned_data_set.parquet"  # Modify this for Q7
...
def load_reference_data(filename):
    ...
    # ]
    reference_data = reference_data[['Partner ISO', 'Commodity Code', 'Year', 'Trade Value (US$)']]
    reference_data.rename(columns={'Trade Value (US$)': 'target'}, inplace=True) # Needed to have a common target mapping in prefect
    features = ['Partner ISO', 'Commodity Code', 'Year']
...
def fetch_data():
    ...
    df.rename(columns={'Trade Value (US$)': 'target'}, inplace=True) # Needed to have a common target mapping in prefect
    print(df.info())
    return df
...
def run_evidently(ref_data, data):
    ...
    mapping = ColumnMapping(
        ...
        categorical_features=['Partner ISO', 'Commodity Code'],
        datetime_features=[],
...
def save_report(result):
    """Save evidendtly profile for trade value prediction to mongo server"""
...
def save_html_report(result, filename_suffix=None):
    """Create evidently html report file for trade value prediction"""

    if filename_suffix is None:
        filename_suffix = datetime.now().strftime('%Y-%m-%d-%H-%M')

    result.save(f"trade_value_drift_report_{filename_suffix}.html")
